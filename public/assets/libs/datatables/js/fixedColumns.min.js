...
$jscomp.polyfill = function(c, e, g, q) { if (e) { g = $jscomp.global;
        c = c.split("."); for (q = 0; q < c.length - 1; q++) { var k = c[q];
            k in g || (g[k] = {});
            g = g[k] } c = c[c.length - 1];
        q = g[c];
        e = e(q);
...
(function(c) { "function" === typeof define && define.amd ? define(["jquery", "datatables.net"], function(e) { return c(e, window, document) }) : "object" === typeof exports ? module.exports = function(e, g) { e || (e = window);
        g && g.fn.dataTable || (g = require("datatables.net")(e, g).$); return c(g, e, e.document) } : c(jQuery, window, document) })(function(c, e, g, q) {
    ...
    c.extend(p.prototype, {
        ...
        _fnConstruct: function(a) {
            ...
            if ("function" != typeof this.s.dt.oInstance.fnVersionCheck || !0 !== this.s.dt.oInstance.fnVersionCheck("1.8.0")) alert("FixedColumns " + p.VERSION + " required DataTables 1.8.0 or later. Please upgrade your DataTables installation");
                ...
                if (0 < b.s.iLeftColumns) c(b.dom.grid.left.liner).on("mouseover.DTFC touchstart.DTFC",
                    function() { h || "key" === d || (d = "left") }).on("scroll.DTFC", function(a) {!d && a.originalEvent && (d = "left"); "left" === d && (b.dom.scroller.scrollTop = b.dom.grid.left.liner.scrollTop, 0 < b.s.iRightColumns && (b.dom.grid.right.liner.scrollTop = b.dom.grid.left.liner.scrollTop)) }).on(f, function(a) { d = "left";
                ...
                if (0 < b.s.iRightColumns) c(b.dom.grid.right.liner).on("mouseover.DTFC touchstart.DTFC", function() { h || "key" === d || (d = "right") }).on("scroll.DTFC",
                    function(a) {!d && a.originalEvent && (d = "right"); "right" === d && (b.dom.scroller.scrollTop = b.dom.grid.right.liner.scrollTop, 0 < b.s.iLeftColumns && (b.dom.grid.left.liner.scrollTop = b.dom.grid.right.liner.scrollTop)) }).on(f, function(a) { d = "left";
                    b.dom.scroller.scrollLeft -= "wheel" === a.type ? -a.originalEvent.deltaX : a.originalEvent.wheelDeltaX });
                c(e).on("resize.DTFC", function() { b._fnGridLayout.call(b) });
                var m = !0,
                    l = c(this.s.dt.nTable);
                l.on("draw.dt.DTFC", function() { b._fnColCalc();
                    b._fnDraw.call(b, m);
                    m = !1 }).on("key-focus.dt.DTFC",
                    function() { d = "key" }).on("column-sizing.dt.DTFC", function() { b._fnColCalc();
                    b._fnGridLayout(b) }).on("column-visibility.dt.DTFC", function(a, c, d, f, h) { if (h === q || h) b._fnColCalc(), b._fnGridLayout(b), b._fnDraw(!0) }).on("select.dt.DTFC deselect.dt.DTFC", function(a, c, d, f) { "dt" === a.namespace && b._fnDraw(!1) }).on("position.dts.dt.DTFC", function(a, d) { b.dom.grid.left.body && c(b.dom.grid.left.body).find("table").eq(0).css("top", d);
                    b.dom.grid.right.body && c(b.dom.grid.right.body).find("table").eq(0).css("top", d) }).on("destroy.dt.DTFC",
                    function() { l.off(".DTFC");
        ...
        _fnColCalc: function() {
            ...
            c.each(this.s.dt.aoColumns, function(h, f) {
                ...
                if (f.filter(":visible").length) {
                    var m =
                        f.outerWidth();
                    if (0 === a.s.aiOuterWidths.length) { var l = c(a.s.dt.nTable).css("border-left-width");
                        m += "string" === typeof l && -1 === l.indexOf("px") ? 1 : parseInt(l, 10) } a.s.aiOuterWidths.length === a.s.dt.aoColumns.length - 1 && (l = c(a.s.dt.nTable).css("border-right-width"), m += "string" === typeof l && -1 === l.indexOf("px") ? 1 : parseInt(l, 10));
        ...
        _fnClone: function(a, b, d, h) {
            ...
            } else { g = this._fnCopyLayout(e.aoHeader, d, !1); var p = [];
                e.oApi._fnDetectHeader(p, c(">thead", a.header)[0]);
                n = 0; for (m = g.length; n < m; n++) { var t = 0; for (k = g[n].length; t < k; t++) p[n][t].cell.className = g[n][t].cell.className, c("span.DataTables_sort_icon", p[n][t].cell).each(function() { this.className = c("span.DataTables_sort_icon", g[n][t].cell)[0].className }) } } this._fnEqualiseHeights("thead", this.dom.header, a.header);
            ...
            } else c(">tbody>tr", f.dom.body).each(function(a) { r = this.cloneNode(!0);
            ...
            this._fnEqualiseHeights("tbody",
                f.dom.body, a.body);
            if (null !== e.nTFoot) {
                if (h) { null !== a.footer && a.footer.parentNode.removeChild(a.footer);
            ...
            c(b).each(function(a) { v = d[a];
                this.style.width = f.s.aiInnerWidths[v] + "px" });
            null !== f.s.dt.nTFoot && (b = e.oApi._fnGetUniqueThs(e, c(">tfoot", a.footer)[0]), c(b).each(function(a) { v = d[a];
                this.style.width = f.s.aiInnerWidths[v] + "px" }))
        },
        ...
        _fnEqualiseHeights: function(a, b, d) {
            if ("none" != this.s.sHeightMatch || "thead" === a || "tfoot" === a) {
                ...
                var f = [];
                d = 0;
                for (a = b.length; d < a; d++) { var g = e[d].offsetHeight; var k = b[d].offsetHeight;
        ...
        _firefoxScrollError: function() { if (u === q) { var a = c("<div/>").css({ position: "absolute", top: 0, left: 0, height: 10, width: 50, overflow: "scroll" }).appendTo("body");
    ...
    k.Api.register("fixedColumns().cellIndex()", function(a) { a = c(a); if (a.parents(".DTFC_Cloned").length) { var b = a.parent().index();
